...
export default function PagesEdit({ page, parentPages }: PagesEditProps) {
    ...
    function handleUnpublish() {
        router.put(`/admin/pages/${page.id}`, { ...data, status: 'draft' });
    }
    ...
    function handleReject(reason: string) {
        router.post(`/admin/pages/${page.id}/reject`, { reason });
            ...
            header={
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {page.status === 'draft' && (
                            <button type="button" onClick={handleSubmitForReview} className="rounded-lg bg-orange-500 px-4 py-2 text-sm font-medium text-white hover:bg-orange-600 transition-colors">Soumettre pour revue</button>
                        )}
        ...
        >
            ...
            <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6">
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label htmlFor="parent_id" className="block text-sm font-medium text-gray-700">Page parente</label>
                            <select id="parent_id" value={data.parent_id} onChange={(e) => setData('parent_id', e.target.value ? Number(e.target.value) : '')} className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
