...
export default function PagesIndex({ pages, filters }: PagesIndexProps) {
    ...
    function handleDelete(page: PageData) {
        if (!confirm(`Êtes-vous sûr de vouloir supprimer la page "${page.title}" ?`)) return;
    ...
    function handleForceDelete(page: PageData) {
        if (!confirm(`Êtes-vous sûr de vouloir supprimer definitivement la page "${page.title}" ? Cette action est irreversible.`)) return;
        router.delete(`/admin/pages/${page.id}/force-delete`);
    }

    function handleEmptyTrash() {
        if (!confirm('Êtes-vous sûr de vouloir vider la corbeille ? Toutes les pages seront supprimees definitivement.')) return;
